...
public:
	...
	Coord3D getSize3D(void)const{ return Coord3D(_xSize, _ySize, _zSize);};
	...
	void window_counts_inplace(int window_size);
...
template <typename T>
T& Grid3D <T>::operator[](unsigned int x){
	return _data[x];
}
...
template <typename T>
void Grid3D < T >::window_counts_inplace(int window_size) {
	for (int d1 = 0; d1 < 3; d1++) {
		int d2 = (d1 + 1) % 3;
		int d3 = (d1 + 2) % 3;
		Coord3D c;
		for (int c1 = 0; c1 < getSize3D()[d1]; c1++) {
			c[d1] = c1;
			for (int c2 = 0; c2 < getSize3D()[d2]; c2++) {
				c[d2] = c2;

				int current_window_sum = 0;
				int window_values[window_size];
				for (int i = 0; i < window_size; i++) {
					window_values[i] = 0;
				}

				for (int c3 = 0; c3 < getSize3D()[d3]; c3++) {
					c[d3] = c3;
					int value = (*this)(c);
					current_window_sum += value;
					current_window_sum -= window_values[c3 % window_size];
					window_values[c3 % window_size] = value;
					(*this)(c) = current_window_sum;
				}
			}
		}
	}
