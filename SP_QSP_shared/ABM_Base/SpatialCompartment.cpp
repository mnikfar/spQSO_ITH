...
std::string SpatialCompartment::compartment_cells_to_string(void) const{
	std::stringstream ss;
	// header
	//ss << "x,y,z,Type,State,Diameter" << getExtraRemarkHeader() << endl;
	ss << "x,y,z,Type,State,Diameter,"<< getExtraRemarkHeader()<<endl;
	float rx,ry,rz;
	...
	for (CellVec::size_type i = 0 ; i < _vecAgent.size(); i++)
	{
		...
		*/
		...
		//	ss << ptr->getAdjustedX()+0.5 << "," << ptr->getAdjustedY()+0.5
		// 	<< "," << ptr->getAdjustedZ()+0.5 << "," << ptr->getType() 
		// 	<< "," << ptr->getState() <<"," << dia <<","<< ptr->getRemark()<<endl;
		//}else{
    	rx = -0.5 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(0.5+0.5)));
    	ry = -0.5 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(0.5+0.5)));
    	rz = -0.5 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(0.5+0.5)));
    	//std::cout<<rx<<" "<<ry<<" "<< rz<<endl;
			ss << ptr->getAdjustedX()+0.5+rx << "," << ptr->getAdjustedY()+0.5+ry
		 	<< "," << ptr->getAdjustedZ()+0.5+rz << "," << ptr->getType() 
		 	<< "," << ptr->getState() <<"," << dia << ","<< ptr->getRemark()<<endl;
		//}
...
std::string SpatialCompartment::compartment_cells_to_stringa(void) const{
	std::stringstream ss;
	// header
	ss << "x,y,z,Type,State"<<endl;
	for (CellVec::size_type i = 0 ; i < _vecAgent.size(); i++)
	{
		CellAgent * ptr = _vecAgent[i];
		ss << ptr->getAdjustedX() << "," << ptr->getAdjustedY()
			<< "," << ptr->getAdjustedZ() << "," << ptr->getType()<<"," << ptr->getState()  
			<<endl;
	}
	return ss.str();
