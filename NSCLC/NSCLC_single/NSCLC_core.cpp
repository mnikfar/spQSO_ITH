...
void NSCLC_Core::writeGrids(unsigned long slice, unsigned int option){
	...
	{
		{
			std::ofstream& snap = output_hub.getNewGridToSnapshotStream(slice, "cell_",1);
			snap << _tumor.compartment_cells_to_string();
			snap.close();
		}
		{
			std::ofstream& snap = output_hub.getNewGridToSnapshotStream(slice, "score_",1);
			snap << _tumor.compartment_cells_to_stringa();
			snap.close();
		}		
		{
			std::ofstream&  snap = output_hub.getNewGridToSnapshotStream(slice, "cyto_",2);
	...
	{
		{
			std::ofstream&  snap = output_hub.getNewGridToSnapshotStream(slice, "grid_core_",3);
			snap << _tumor.printGridToFile();
			snap.close();
