...
void Tumor::time_slice_recruitment(){
	...
	int countc;
	Coord crd_p;
	double lx=params.getVal(SP_QSP_IO::SP_QSP_NSCLC::PARAM_TUMOR_X);
	double ly=params.getVal(SP_QSP_IO::SP_QSP_NSCLC::PARAM_TUMOR_Y);
	double lz=params.getVal(SP_QSP_IO::SP_QSP_NSCLC::PARAM_TUMOR_Z);
	for (auto& crd : _ta_source){	
		double ncc = 0; 
		for (int i = -2; i < 2; i++){
			for (int j = -1; j < 1; j++){
				for (int k = -2; k < 2; k++){						
					...
					crd_p.y = crd.y + j;
					crd_p.z = crd.z + k;
						if ((crd_p.x >0 && crd_p.x < lx) && (crd_p.y > 0 && crd_p.y < ly)&& (crd_p.z > 0 && crd_p.z < lz)){
